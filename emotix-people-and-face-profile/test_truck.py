...
parser.add_option("-o", "--output_file", dest="output_file", help="Path to output video file.", default="/home/vysakh/Accubits/keras-frcnn-master/output.mp4")
parser.add_option("-d", "--input_dir", dest="input_dir", help="Path to input working directory.", default="/home/vysakh/Accubits/keras-frcnn-master/input/")
parser.add_option("-u", "--output_dir", dest="output_dir", help="Path to output working directory.", default="/home/vysakh/Accubits/keras-frcnn-master/output/")
...
def main():
	...
	for img_name in list_files:
		...
		for key in bboxes:
				if key == 'person':
					...
					for jk in range(new_boxes.shape[0]):
						...
						cv2.line(img_scaled,(624,570),(624,50),(0,0,255),2)

						cv2.rectangle(img_scaled,(x1, y1), (x2, y2), class_to_color[key],2)
						crop_img = img_scaled[y1:y2,x1:x2]
						cv2.imwrite("Detected Subjects/subject"+str(cnt)+".png", crop_img)
						cnt +=1
						#if w > 20 and h > 25:
								#cv2.rectangle(img_scaled, (x,y), (x+w,y+h), (180, 1, 0), 1)
						w=x2-x1
						h=y2-y1
						x_1=w/2
						y_1=h/2
						...
						cy=y1+y_1
						centroid=(cx,cy)
				    		cv2.circle(img_scaled,(int(cx),int(cy)),4,(0,255,0),-1)
						if cx >= 620 and cy <=630:
							counts += 1
		...
		cv2.putText(img_scaled, "Static Object Count: " + str(list(accumulate(all_objects)))+"    Moving Object Count:"+str(counts), (5, 19), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 255, 255), 1)
